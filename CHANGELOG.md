...
- Dummy Tailwind `@source` list now also covers rbenv-installed git gems (`~/.rbenv/versions/*/lib/ruby/gems/*/bundler/gems/{flatpack,RecordingStudio}*`), so FlatPack and Recording Studio utility classes (sidebar layout, pricing grid) compile when the host uses rbenv rather than `vendor/bundle`, Docker's `/usr/local/bundle`, or mise. Fixes broken sidebar/card layout in that setup.
- Dummy app `mail` gem bumped 2.9.0 → 2.9.1 to clear a Bundler Audit advisory (GHSA-mvxr-6m87-mv2q). Dev/test dependency only; the packaged gem is unaffected.
