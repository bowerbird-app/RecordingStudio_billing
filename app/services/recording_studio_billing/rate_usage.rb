...
module RecordingStudioBilling
  class RateUsage
    ...
    def initialize(root_recording:, meter_recording:, manifest_digest:, window_starts_at:, window_ends_at:,
                   metadata: {})
    ...
    def call
      ...
      return denied(:no_entitlement) unless EntitlementAccess.for(root_recording: root, account_recording: account,
                                                                  at: window_ends_at).has_feature?(terms.fetch(:meter).fetch("usage_key"))
    ...
    attr_reader :manifest_digest, :metadata, :meter_recording_input, :root_recording_input, :window_ends_at,
                :window_starts_at
    ...
    def authority!
      ...
      raise ArgumentError, "billing account must belong directly to the normalized root" unless account.recordable_type == "RecordingStudioBilling::Account" && account.root_recording_id == root.id && account.parent_recording_id == root.id

      unless window_starts_at.respond_to?(:to_time) && window_ends_at.respond_to?(:to_time) && window_ends_at.to_time > window_starts_at.to_time
        raise ArgumentError,
              "rating window is invalid"
      end
    ...
    def rating_terms(manifest)
      ...
      customer_rates = rating.fetch("customer_rates").values.select do |price|
        price["usage_unit_recording_id"] == unit_id
      end
    ...
    def aggregate(root, account, terms, events)
      ...
      MeterAggregation.find_or_create_by!(attributes.slice(:root_recording, :account_recording, :meter_recording_id,
                                                           :window_starts_at, :window_ends_at, :manifest_digest, :input_digest)) do |row|
    ...
    def rated_attributes(root, account, aggregation, terms)
                                                ...
                                                }, allow_authoritative_totals: true)
      {
        ...
        quantity:, customer_amount_minor: nil, customer_currency_code: nil, customer_currency_exponent: nil,
    ...
    def money_for(quantity, terms, package:)
      ...
      multiplier = if package && terms.fetch("pricing_model") == "package"
                     ...
                     unless size.positive? && (quantity % size).zero?
                       raise ArgumentError,
                             "package quantity is non-integral"
                     end
      ...
      { amount_minor: amount * multiplier, currency_code: terms.fetch("currency_code"),
        currency_exponent: Integer(terms.fetch("currency_exponent")) }
    end
    ...
    def retry_existing_result
      aggregation = MeterAggregation.where(manifest_digest:, window_starts_at: window_starts_at.to_time,
                                           window_ends_at: window_ends_at.to_time).order(created_at: :desc).first
