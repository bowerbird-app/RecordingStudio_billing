...
The dummy catalogue nominates `demo_api_calls` (a 1000-call pack plus a metered service with 5 included). After seed usage of 11 API calls, `remaining_credits` for `demo_api_calls` is the live pack leftover (989). The gem suite covers a live plan allocation plus packs, and a second meter that stays isolated.
