...
- This repository is the Recording Studio Billing engine (`recording_studio_billing`).
- Preserve engine namespace isolation under `RecordingStudioBilling`.
- Host setup for workers, Stripe restricted keys, API version pinning, and tax is in `docs/billing.md` and `README.md`.
...
- Add focused regression tests for engine hooks, generators, Recording Studio integration points, and host-facing contract changes.

## Repo Conventions

- Keep internal dependency assumptions intact unless the request explicitly asks to change private gem infrastructure.
- Update docs when host setup or engine behavior changes.
