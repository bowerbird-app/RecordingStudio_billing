...
- FlatPack UI: sign-in uses the gem-template `application` layout with `html data-theme="rounded"`; signed-in dummy and billing pages use core `recording_studio/default_layout` (`UsesDefaultLayout`) with PageNav back/close and no Sign out or Root Switchable control in that slot. Dummy sets rounded on `html` via `_default_layout_head` (not a forked layout). Dummy pins Flatpack `v0.1.141` so rounded buttons get charcoal primary and 1rem radius from Flatpack tokens, not a Billing CSS fork.
