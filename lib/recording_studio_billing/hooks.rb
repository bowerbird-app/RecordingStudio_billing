...
module RecordingStudioBilling
  ...
  #   RecordingStudioBilling.configuration.hooks.after_initialize do
  #     Rails.logger.info "RecordingStudioBilling initialized!"
  #   end
  #
  # @example Running hooks
  #   RecordingStudioBilling::Hooks.run(:after_initialize)
  #
  class Hooks
    ...
    def log_hook_error(error, event_name, _hook)
      ...
      Rails.logger.error "[RecordingStudioBilling::Hooks] Error in #{event_name} hook: #{error.message}"
    ...
    class << self
      ...
      # @return [Array] Results from all hooks
      def run(event_name, *)
        RecordingStudioBilling.configuration.hooks.run(event_name, *)
      end
      ...
      def run_around(event_name, context, &)
        RecordingStudioBilling.configuration.hooks.run_around(event_name, context, &)
      end
