  ...
  module Services
    ...
    class BaseService
      ...
      def run_before_hooks
        ...
        RecordingStudioBilling::Hooks.run(:before_service, self.class, service_args)
      end
      ...
      def run_after_hooks(result)
        ...
        RecordingStudioBilling::Hooks.run(:after_service, self.class, result)
      end
      ...
      def run_with_around_hooks
        if hooks_enabled? && RecordingStudioBilling.configuration.hooks.registered?(:around_service)
          RecordingStudioBilling::Hooks.run_around(:around_service, self) { perform }
        else
          perform
        end
      end

      # Check if hooks are enabled (RecordingStudioBilling is loaded and configured)
      #
      # @return [Boolean]
      def hooks_enabled?
        defined?(RecordingStudioBilling) &&
          RecordingStudioBilling.respond_to?(:configuration) &&
          RecordingStudioBilling.configuration.respond_to?(:hooks)
      end
