...
8. For an admin root, include `RecordingStudioBilling::BillingAdminSupport` and enable `:accessible` on that root. Mount with `recording_studio_admin_for`. The concern uses `RecordingStudioAdmin::AllowsAdminSections` to register `:root`, `:billing_commercial`, `:billing_financial`, and `:billing_operations`.
