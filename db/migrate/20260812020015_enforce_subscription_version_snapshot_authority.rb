...
class EnforceSubscriptionVersionSnapshotAuthority < ActiveRecord::Migration[8.1]
  def up
    execute <<~SQL
      CREATE OR REPLACE FUNCTION rs_billing_protect_subscription_item_version() RETURNS trigger AS $$
